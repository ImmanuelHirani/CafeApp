...
@section('content')
    <section id="custom-order"
        ...
        <div class="relative flex flex-col col-span-7 gap-4 wrap">
            ...
            <div class="flex flex-col overflow-hidden accordion-wrapper">
                <div class="accordion-head">
                    <div class="flex items-center justify-between pb-4 cursor-pointer lg:pb-6 head-wrapper-accordion">
                ...
                @isset($sizes)
                    @if ($sizes->isNotEmpty())
                        <div class="text-xl transition-all border-b-[1px] duration-300 ease-in-out accordion-content max-h-0">
                            <div class="flex flex-wrap items-center px-0.5 gap-3 pb-6 mt-3 lg:mt-6 wrap">
                                @foreach ($sizes as $size)
                                    <button data-id="{{ $size->size_ID }}" data-price="{{ $size->price }}"
                                        ...
                                        class="w-[20%] p-3 hover:bg-secondary-color transition-all ease-in-out duration-300 size-button rounded-full outline outline-white/30 ">
                                        {{ $size->size }}
                                    </button>
                                @endforeach
                            </div>
            ...
            @foreach ($categories as $category)
                <div class="flex flex-col px-1 overflow-hidden accordion-wrapper">
                    <div class="accordion-head">
                        <div class="flex items-center justify-between pb-3 cursor-pointer lg:pb-6 head-wrapper-accordion">
                                ...
                                @foreach ($category->properties as $property)
                                    @if ($property->is_active == 1)
                                        <button data-price="{{ $property->price }}"
                                            ...
                                            class="px-8 py-3 text-sm transition-all duration-300 ease-in-out rounded-full hover:bg-secondary-color topping-button w-fit outline  outline-white/30 3xl:text-xl lg:text-xl">
        ...
        <img src="../asset/CustomOrder.png" class="relative top-0 w-full col-span-5 row-span-5 wrap lg:sticky lg:top-36"
            alt="" />
        <div class="container fixed z-50 w-full text-white translate-x-1/2 right-1/2 bottom-4 add-to-cart-custom-order">
            <form action="{{ route('store.custom.order') }}" method="POST"
                ...
                <div class="flex flex-col items-center justify-between lg:w-[20%] w-full gap-3 rounded-lg add-to-cart">
